...
def process_body(idx, total, body, docstring, data, docs_words):
    errors = 0
    ...
    try:
        ...
        docs_words.append(doc_tokenizer(docstring))
...
def process_data(inputs, outputs, singlethreaded):
    ...
    if singlethreaded:
        for idx, (body, docstring) in enumerate(zip(inputs, outputs)):
            errors += process_body(idx, total, body,
                                   docstring, data, docs_words)
    else:
        num_cores = multiprocessing.cpu_count()
        _errors = Parallel(n_jobs=2*num_cores)(
            delayed(process_body)(idx, total, body, docstring, data, docs_words) for idx, (body, docstring) in enumerate(zip(inputs, outputs)))
        errors = sum(_errors)

    print("Generated %d graphs and had %d errors out of %d snippets" %
          (len(data), errors, len(inputs)))
...
def main(input, output, singlethreaded):
    ...
    graphs, docs = process_data(code, docstring, singlethreaded)

    with open(output, 'wb') as output_file:
        pickle.dump(zip(graphs, docs), output_file)
